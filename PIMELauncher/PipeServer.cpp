...
static constexpr UINT ID_SETTINGS = 1003;
...
LRESULT PipeServer::wndProc(UINT msg, WPARAM wp, LPARAM lp) {
	...
	case WM_COMMAND:
		...
		case ID_SHOW_DEBUG_LOGS:
			...
			break;
        case ID_SETTINGS:
            ShowSettingsDialog(GetModuleHandle(NULL), hwnd_);
            break;
		}
		break;
	default:
...
LPCTSTR PipeServer::registerWndClass(WNDCLASSEX& wndClass) const {
	...
	wndClass.lpfnWndProc = [](HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) -> LRESULT {
		if (msg == WM_NCCREATE) {
			...
			auto pObj = reinterpret_cast<PipeServer*>(::GetWindowLongPtr(hwnd, GWLP_USERDATA));
			if (pObj) {
