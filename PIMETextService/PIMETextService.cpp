...
TextService::TextService(ImeModule* module):
	...
	candFontSize_(12),
    candColor_(GetSysColor(COLOR_WINDOWTEXT)),
    candBkColor_(GetSysColor(COLOR_WINDOW)) {

    // Load settings
    wstring settingsPath = getAppLocalDir() + L"\\PIME\\PIMESettings.json";
    Json::Value root;
    if (loadJsonFile(settingsPath, root)) {
        candFontSize_ = root.get("fontSize", 12).asInt();
        candColor_ = (COLORREF)root.get("textColor", (Json::UInt)GetSysColor(COLOR_WINDOWTEXT)).asUInt();
        candBkColor_ = (COLORREF)root.get("bkColor", (Json::UInt)GetSysColor(COLOR_WINDOW)).asUInt();
        if (root.get("horizontal", false).asBool()) {
            candPerRow_ = 10;
        } else {
            candPerRow_ = 1;
        }
